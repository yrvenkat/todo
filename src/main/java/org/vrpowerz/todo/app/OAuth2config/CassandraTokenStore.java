...
import org.vrpowerz.todo.model.OAuth2AccessTokenBuilder;
import org.vrpowerz.todo.repository.OAuth2AccessTokenRepository;
import org.vrpowerz.todo.repository.OAuth2RefreshTokenRepository;
...
public class CassandraTokenStore implements TokenStore {
    ...
    @Autowired
    public void setTokenRepository(OAuth2AccessTokenRepository tokenRepository) {
    ...
    public OAuth2Authentication readAuthentication(String token) {
        ...
        final org.vrpowerz.todo.model.OAuth2AccessToken auth2AccessToken = tokenRepository.findById( extractTokenKey(token) ).get();
        try {
    ...
    public void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
        ...
        final OAuth2AccessTokenBuilder accessTokenBuilder = new OAuth2AccessTokenBuilder();
    ...
    public OAuth2AccessToken readAccessToken(String tokenValue) {
        final org.vrpowerz.todo.model.OAuth2AccessToken auth2AccessToken = tokenRepository.findById( extractTokenKey(tokenValue) ).orElse(null);
    ...
    public void removeAccessToken(String tokenValue) {
        tokenRepository.deleteById( extractTokenKey(tokenValue) );
    }

    @Override
    public void storeRefreshToken(OAuth2RefreshToken refreshToken, OAuth2Authentication authentication) {
        refreshTokenRepository.save(new org.vrpowerz.todo.model.OAuth2RefreshTokenBuilder()
    ...
    public OAuth2RefreshToken readRefreshToken(String tokenValue) {
        final org.vrpowerz.todo.model.OAuth2RefreshToken refreshToken = refreshTokenRepository.findById( extractTokenKey(tokenValue) ).get();
    ...
    public void removeRefreshToken(final String tokenValue) {
        refreshTokenRepository.deleteById( extractTokenKey(tokenValue) );
    }
    ...
    public void removeAccessTokenUsingRefreshToken(final String refreshTokenValue) {
        final Select select = QueryBuilder.select("token_object").from("oauth2_access_tokens");
        select.where(QueryBuilder.eq("refresh_token", extractTokenKey(refreshTokenValue) ));
//        Conversion error in spring.data framework. Hence manually converting
//        final List<ByteBuffer> tokens = cassandraOperations.select(select, ByteBuffer.class);
//        tokens.forEach(t -> {
//            final OAuth2AccessToken accessToken = deserializeAccessToken(t.array());
//            tokenRepository.deleteById( accessToken.getValue() );
        List<Row> tokens = cassandraOperations.select(select, Row.class);
        tokens.forEach(t -> {
            final OAuth2AccessToken accessToken = deserializeAccessToken(t.getBytes("token_object").array());
            tokenRepository.deleteById( accessToken.getValue() );
        });
    }
...
//        Conversion from Row to ByteBuffer error in framework (Fix available only on 2.1.13). 
//        Hence extracted it from Row  
//        final ByteBuffer token =  cassandraOperations.selectOne(select,ByteBuffer.class);
		Row r = cassandraOperations.selectOne(select, Row.class);
		final ByteBuffer token = (r != null) ? r.getBytes("token_object") : null;
...
//        return cassandraOperations.select(select, ByteBuffer.class).stream().map( it -> deserializeAccessToken(it.array()) ).collect(Collectors.toList());
        return cassandraOperations.select(select, Row.class).stream().map( it -> deserializeAccessToken(it.getBytes("token_object").array()) ).collect(Collectors.toList());
    }
